...
%% 2) Organise Folder into Image Stacks and channels 

%Transform channels so they roughly overlay each othereg
% [Chan1 Vert Flip,Chan1 Horz Flip, Chan 1 Rotate;
%[Chan2 Vert Flip,Chan2 Horz Flip, Chan 2 Rotate]
% 1 to flip, 0 to not, rotate = 0,90,180,270
imStackChannelTransforms = [0 0 0;0 0 0;0 0 0];

imStackMetadata = true ; % Images are usually split using embedded OME metadata but can be disabled if this causes problems
imStackNumberOfChannels = 3; % Input the number of channels in the data
imStackMultipleFiles = false ; % choose this if you don't have metadata and you're stack is split over multiple tiff files (i.e. >4Gb)

% Don't touch from here

Tiff_Channel_Splitter(inputFolder,imStackChannelTransforms,imStackMetadata,imStackNumberOfChannels,imStackMultipleFiles);

%Detect all folder made that have the raw stacks in them

disp('Organization completed');
%%
sysVar.allFolders = arrayfun(@(x)[sysVar.fileName,x.name],dir(completeName),'UniformOutput',false); 
...
sysVar.cmd = [sysConst.JIM,'Align_Channels',sysConst.fileEXE,' "',workingDir,'Alignment"',sysVar.allChannelNames,' -Start ',num2str(alignStartFrame),' -End ',num2str(alignEndFrame),' -MaxShift ',num2str(alignMaxShift)];
...
elseif length(split(detectWeights))<imStackNumberOfChannels
    ...
    if ~ischar(detectionStartFrame)
        detectionStartFrame = num2str(detectionStartFrame);
    end

    if ~ischar(detectionEndFrame)
    ...
    if traceIdx<=size(sysVar.traces1,1)
        ...
        if montage.showStepfit && stepfitEnable
            ...
            for j=1:size(sysVar.traces1,2)
                if ismember(j-1,sysVar.stepPoints(traceIdx,:))
                    sysVar.count = sysVar.count +1;
                end
                sysVar.stepPlot(j) = sysVar.stepMeans(traceIdx,sysVar.count);
            end
            if stepfitChannel == 1
