...
if insubfolders
    for i=1:size(allfiles,1)
        ...
        for j=1:size(innerfolder,1)
            if size(dir([pathname,allfiles(i).name,'\',innerfolder(j).name,'\Traces_Channel_1_traces.csv']),1)==1
                channel1 = [channel1 [pathname,allfiles(i).name,'\',innerfolder(j).name,'\Traces_Channel_1_traces.csv']];
            end
        end
    end
else
    for i=1:size(allfiles,1)
        if size(dir([pathname,allfiles(i).name,'\Traces_Channel_1_traces.csv']),1)==1
            channel1 = [channel1 [pathname,allfiles(i).name,'\Traces_Channel_1_traces.csv']];
        end 
    end
end
...
for filetocheck=1:numofexps
    ...
    numofflour = mean(traces(:,1:5)')./mean(stepheight);;
    ...
    allnumoffluor = allinitvals./mean(allstepheights);
