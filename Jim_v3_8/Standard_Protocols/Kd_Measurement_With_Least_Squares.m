...
%% 6) Fit binding Curve
PB_substrate = 51.38; %exposure corrected PB of substrate
PB_binder = 36.0; %exposure corrected PB of binder
Substrate_labelling_ratio = 40; %degree of labelling of substrate if it is partially labelled or assembled
Binder_labelling_ratio = 1; %./(1./concentration'); %leave 1 if there is no partial labelling during titration, otherwise use concentration' variable

x = concentration';
yx = medianintsenityratio%.*(PB_substrate/PB_binder).*(Binder_labelling_ratio).*(1/Substrate_labelling_ratio);
...
scatter(x,yx)
hold on
title('Kd fitting using median slope')
xlabel(['Concentration ',Concentrationunits]) % x-axis label
ylabel('Intensity ratio of Binder to Substrate') % y-axis label
plot(xout,yout)
hold off
...
x = concentration';
yx = leastsquaresratio.*(PB_substrate/PB_binder).*(Binder_labelling_ratio).*(1/Substrate_labelling_ratio);
...
scatter(x,yx)
hold on
title('Kd fitting using OLS interpolated slope')
xlabel(['Concentration ',Concentrationunits]) % x-axis label
ylabel('Intensity ratio of Binder to Substrate') % y-axis label
plot(xout,yout)
hold off
