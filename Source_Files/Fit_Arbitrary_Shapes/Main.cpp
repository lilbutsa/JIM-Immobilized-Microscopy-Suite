...
int main(int argc, char *argv[])
{
	...
	double boundaryDist = 4.1, backgroundDist = 20;
	...
	BLCSVIO::readVariableWidthCSV(foregroundposfile, labelledpos);
	...
	int masklength = 2 * boundaryDist + 1;
	vector<uint8> initialBoundary(imagePoints, 0), expandedBoundary(imagePoints, 0),mask(masklength*masklength,0);

	IppiMorphState* pSpec = NULL;
	Ipp8u* pBuffer = NULL;
	IppiSize roiSize = { imageWidth, imageHeight };
	IppiSize maskSize = { masklength,masklength };
	int xpos, ypos,iboundaryDist = boundaryDist;
	...
	vector<uint8> alledgeboundaries(imagePoints, 0);
	for (int i = 0; i < expandedpos.size(); i++)for (int j = 0; j < expandedpos[i].size(); j++) alledgeboundaries[expandedpos[i][j]] = 255;
	BLTiffIO::WriteSinglePage1D(alledgeboundaries, output + "_ROIs.tif", imageWidth, 8);

	//read in alternative background

	std::vector<std::vector<int>> backgroundinit(3000, vector<int>(1000, 0));

	BLCSVIO::readVariableWidthCSV(backgroundposfile, backgroundinit);
	backgroundinit.erase(backgroundinit.begin());
	alledgeboundaries = vector<uint8>(imagePoints, 0);
	vector<uint8> expandedbackground(imagePoints, 0);
	...
	vector<uint8>  backgoundBoundary(imagePoints, 0);
	mask.clear();
	...
	vector<uint8> backgroundregion(imagePoints, 0);
	for (int i = 0; i < backgroundpos.size(); i++)for (int j = 0; j < backgroundpos[i].size(); j++) backgroundregion[backgroundpos[i][j]] = 255;
	BLTiffIO::WriteSinglePage1D(backgroundregion, output + "_Background_Regions.tif", imageWidth, 8);
