...
int main(int argc, char *argv[])
{
	...
	double mean, stddev;
	vector<uint8> detected(imagePoints, 0);
	...
	BLTiffIO::WriteSinglePage1D(detected, output + "_Regions.tif", imageWidth, 8);
	...
	vector<uint8> filtereddetected(imagePoints, 0);
	for (int i = 0; i < filteredpos.size(); i++)for(int j=0;j<filteredpos[i].size();j++) filtereddetected[filteredpos[i][j]] = 255;

	BLTiffIO::WriteSinglePage1D(filtereddetected, output + "_Filtered_Regions.tif", imageWidth, 8);
