...
startframe = 1; % First frame in statck to take average from (First frame is 1) 
endframe = 15;  % Last frame in stack to take average up to (Make sure this value is not more then the total number of frames)
...
cmd = [JIM,'AS_Measure_Each_Frame.exe "',completename,'" "',workingdir,'Expanded_ROI_Positions.csv" "',workingdir,'Expanded_Background_Positions.csv" "',workingdir,'Channel_1" -Drifts "',workingdir,'Aligned_Drifts.csv"']; % Generate traces using AS_Measure_Each_Frame.exe and write out with the prefix Channel_1
system(cmd)
...
%% Run this to input parameters if you haven't run the code on a single file
% startframe = 1;
% endframe = 10;
% 
% cutoff=1.2;
...
parfor i=1:filenum(1)
    ...
    cmd = [JIM,'Fit_Arbitrary_Shapes.exe "',workingdir,'Detected_Filtered_Positions.csv" "',workingdir,'Detected_Positions.csv" "',workingdir,'Expanded" -boundaryDist ', num2str(innerradius),' -backgroundDist ',num2str(backgroundradius)];
    system(cmd)
    % 3.6) Calculate Sum of signal and background for each frame
    cmd = [JIM,'AS_Measure_Each_Frame.exe "',completename,'" "',workingdir,'Expanded_ROI_Positions.csv" "',workingdir,'Expanded_Background_Positions.csv" "',workingdir,'Channel_1" -Drifts "',workingdir,'Aligned_Drifts.csv"'];
    system(cmd)
