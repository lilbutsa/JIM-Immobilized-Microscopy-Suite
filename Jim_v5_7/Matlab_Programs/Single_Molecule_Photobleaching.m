...
allResults = zeros(NumberOfFiles+3,16);
...
%% 7) Find Signal to Noise
    ...
    allResults(fileNo,12) = mean(SNR);
     
    end 
    dlmwrite(fileout,allSNR,'-append');
    
    allResults(end,12) = mean(allSNR);
...
%% 8) Initial Particle Intensity Distribution
    ...
    for i=1:NumberOfFiles
        allResults(i,13) = sum(hists(2*i,hists(2*i-1,:)<0.5))./sum(hists(2*i,:));
        allResults(i,14) = sum(hists(2*i,hists(2*i-1,:)>=0.5 & hists(2*i-1,:)<1.5))./sum(hists(2*i,:));
        allResults(i,15) = sum(hists(2*i,hists(2*i-1,:)>=1.5 & hists(2*i-1,:)<2.5))./sum(hists(2*i,:));
        allResults(i,16) = sum(hists(2*i,hists(2*i-1,:)>=2.5))./sum(hists(2*i,:));
    end
    
    allResults(end,13) = sum(hists(end,hists(end-1,:)<0.5))./sum(hists(end,:));
    allResults(end,14) = sum(hists(end,hists(end-1,:)>=0.5 & hists(end-1,:)<1.5))./sum(hists(end,:));
    allResults(end,15) = sum(hists(end,hists(end-1,:)>=1.5 & hists(end-1,:)<2.5))./sum(hists(end,:));
    allResults(end,16) = sum(hists(end,hists(end-1,:)>=2.5))./sum(hists(end,:)); 
...
%% 9) Create Combined Figure and Table
    ...
    t.ColumnName = {'Num of Particles','Num of Single Steps','Bleach Rate (1/frames)','Half Life (frames)','10% Bleached (frames)','Gauss Fit Mean', 'Gauss Fit Std. Dev.','Mean Step Height', 'Std. Dev. Step Height','Median Step Height','Mode Step Height','Mean Signal to Noise','Submonomer Fraction','Monomer Fraction','Dimer Fraction', 'Higher Order Fraction'};
