...
int main(int argc, char *argv[])
{
	...
	BLCSVIO::readVariableWidthCSV(positionsfile, backgroundinit);

	vector<int> filamentlinecount = backgroundinit[0];

	std::vector<std::vector<double>> traces;
	BLCSVIO::readVariableWidthCSV(tracesfile, traces);

	std::vector<std::vector<double>> backgroundtraces;
	BLCSVIO::readVariableWidthCSV(backgroundtracesfile, backgroundtraces);
