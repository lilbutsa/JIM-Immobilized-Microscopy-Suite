...
partialstartframe = 1; % OR you can set the actual numbers here 
partialendframe = 10;
...
mindistfromedge = 10; % Excluded particles closer to the edge than this. Make sure this value is larger than the maximum drift. 25 works well in most cases
...
mineccentricity = 0.7; % Eccentricity of best fit ellipse goes from 0 to 1 - 0=Perfect Circle, 1 = Line. Use the Minimum to exclude round objects. Set it to any negative number to allow all round objects
...
foregrounddist=4.1; % Perpendicular distance of foreground kymograph
backgrounddist = 20; % Perpendicular distance of background kymograph
backgroundexpansion = 4.1; % detected spot expansion
cmd = [JIM,'Kymograph_Positions.exe "',workingdir,'Detected_Filtered_Measurements.csv" "',workingdir,'Detected_Positions.csv" "',workingdir,'Expanded" -boundaryDist ', num2str(foregrounddist),' -backgroundDist ',num2str(backgrounddist),' -backInnerRadius ',num2str(backgroundexpansion)]; % Run Fit_Arbitrary_Shapes.exe on the Detected_Filtered_Positions and output the result with the prefix Expanded
system(cmd)
