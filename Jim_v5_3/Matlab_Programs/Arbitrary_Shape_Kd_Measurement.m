...
if insubfolders
    ...
    for i=1:size(allfiles,1)
        if size(dir([pathname,allfiles(i).name,'\Channel_1_Flourescent_Intensities.csv']),1)==1
            ...
            channel1b = [channel1b [pathname,allfiles(i).name,'\Channel_1_Flourescent_Backgrounds.csv']];
             channel2b = [channel2b [pathname,allfiles(i).name,'\Channel_2_Flourescent_Backgrounds.csv']]
        end 
    end
end
...
Concentrationunits = 'uM';
 concentration = [1];%[0.2 0.2 0.2 1 1 1 10 10 10 0.5 0.5 0.5 5 5 5 0.1 0.1 0.1 20 20 20];

%% 5) Measure the ratio of flourescence between channels for each area 
channel1reference = true;
pagenumber = 1;
