...
int main(int argc, char *argv[])
{
	...
	for (int i = 2; i < argc && std::string(argv[i]) != "-Alignment" && std::string(argv[i]) != "-Start"&& std::string(argv[i]) != "-End"; i++) numInputFiles++;
	...
	for (int i = 0; i < numInputFiles; i++)vcinput[i] = BLTiffIO::TiffInput(inputfiles[i]);
	...
	for (int i = 1; i < argc; i++) {
		...
		if (std::string(argv[i]) == "-Start") {
			if (i + 1 < argc) {
				...
				start = max(start, 0);
				cout << "Calculating initial mean starting from " << start + 1 << endl;
			}
		...
		if (std::string(argv[i]) == "-End") {
			if (i + 1 < argc) {
				...
				cout << "Calculating initial mean up to frame " << end << endl;
			}
	...
	if (inputalignment) {
		...
		for (int channelcount = 0; channelcount < numInputFiles; channelcount++) {
			...
			for (int i = 0; i < start; i++)vcinput[channelcount].get1dimage(image1);
			...
			ippiMulC_32f_C1IR(1.0 / (end - start + 1), meanimage[channelcount].data(), srcStep, roiSize);
			adjustedOutputFilename = outputfile + "_initial_mean_" + to_string(channelcount + 1) + ".tiff";

			BLTiffIO::TiffOutput(adjustedOutputFilename, imageDepth, imageWidth, imageHeight).Write1DImage(meanimage[channelcount]);
	...
	for (int channelcount = 0; channelcount < numInputFiles; channelcount++) {
		...
		if (channelcount > 0) {
			...
			BLTiffIO::TiffOutput(adjustedOutputFilename, imageDepth, imageWidth, imageHeight).Write1DImage(translated);
