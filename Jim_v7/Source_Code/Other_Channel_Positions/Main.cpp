...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		if (std::string(argv[i]) == "-positions") {
			if (i + 1 < argc) {
				...
				bpositionslist = true;
				BLCSVIO::readVariableWidthCSV(positionsfile, positionslistin,headerLine);
		...
		if (std::string(argv[i]) == "-backgroundpositions") {
			if (i + 1 < argc) {
				...
				bbacklist = true;
				BLCSVIO::readVariableWidthCSV(positionsfile, backpositionslistin,headerLine);
	...
	BLCSVIO::readCSV(channelalignfile, channelalignin,headerLine);
	BLCSVIO::readCSV(driftfile, driftsin,headerLine);
	BLCSVIO::readCSV(meaurementsfile, positionsin,headerLine);
