...
void driftCorrectMultiChannel(vector<string>& inputfilename, int& start, int& end, int iterations, vector<float>&angle, vector<float>&scale, vector<float>&xoffset, vector<float>&yoffset,vector< vector<float> >& initialmeanimage, vector< vector<float> >& finalmeanimage, vector< vector<float> > &driftsout, int& imageWidth, float maxShift, vector< BLTiffIO::TiffOutput*>& outputFiles) {
	...
	for (int i = 0; i < numInputFiles; i++) {
		vis[i] = new BLTiffIO::TiffInput(inputfilename[i]);
	}
	...
	for (int chancount = 0; chancount < numInputFiles; chancount++)
		for (int imcount = 0; imcount < vis[0]->numOfFrames; imcount++) {
			...
			if (outputFiles.size() > 0 ) {
				outputFiles[chancount]->write1dImage(imaget);
			}
