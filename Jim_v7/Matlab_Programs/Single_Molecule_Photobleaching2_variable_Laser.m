...
for i=1:NumberOfFiles
   for j= uniqueInts
       if contains(channel1{i},['_' num2str(j) 'mW_']) 
           laserInts(i) = j;
       end
   end
end
...
figure
scatter(laserInts,allResults(:,1))
...
for fileNo = 1:length(uniqueInts)    
    ...
    by3 = @(b,bx)(middleThird(100.*conv([zeros(1,length(bx)) normcdf(log(bx),b(1),b(2)) (1+zeros(1,length(bx)))],normpdf(bx,max(bx)/2,b(3)),'same')));
