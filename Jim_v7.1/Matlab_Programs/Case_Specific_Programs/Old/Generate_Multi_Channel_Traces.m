...
if useMetadataFile
    ...
    cmd = [JIM,'TIFF_Channel_Splitter',fileEXE,' "',workingDir,'Images" "',completeName,'" -MetadataFile "',metaFileName,'"']; % Run TIFFChannelSplitter',fileEXE,' using the metadata file and write the split channels to the reults folder with the prefix Images
else
    cmd = [JIM,'TIFF_Channel_Splitter',fileEXE,' "',workingDir,'Images" "',completeName,'" -NumberOfChannels ',num2str(numberOfChannels)];% Run TIFFChannelSplitter',fileEXE,' without the metadata file and write the split channels to the reults folder with the prefix Images
end
system(cmd)
%% 3) Invert Channel
...
%Thresholding
cutoff=1.3; % The cutoff for the initial thresholding
...
maxCount=100;%100; % Maximum number of pixels in a ROI to be counted as a particle. Use this to exclude aggregates
...
maxEccentricity = 0.6;  % Use the maximum to exclude long, thin objects. Set it to a value above 1 to include long, thin objects  
...
if manualAlignment
    channel1Im = im2double(imread([workingDir,'Aligned_aligned_partial_mean_2.tiff']));
else
    channel1Im = im2double(imread([workingDir,'Aligned_initial_partial_mean_2.tiff']));
end
flatim = sort(reshape(channel1Im,[],1),'descend');
fivepclen = round(0.05*length(flatim));
channel1Im = displayMax.*(channel1Im-flatim(end-fivepclen))./(flatim(fivepclen)-flatim(end-fivepclen))+displayMin;
