...
int fitExpZeroOffset(vector<double>& xdata, vector<double>& ydata, vector<double>& paramvec);
...
int main(int argc, char *argv[])
{
	...
	for (int i = 1; i < argc; i++) {
		...
		if (std::string(argv[i]) == "-zeroOffset") {
			useOffset = false;
			cout << "Offset set to zero" << endl;
		}
	...
	for (int i = 0; i + 1 < alldatatofit.size(); i = i + 2) {
		...
		if (useOffset)fitExp(xdata, ydata, allparams[i / 2]);
		else fitExpZeroOffset(xdata, ydata, allparams[i / 2]);
